...
**Build the zip yourself** from the addon's parent directory:

```bash
# Remove pycache first
find plugin.audio.m3sr2019 -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null
...
> **Critical:** Python's standard `logging` module is **silent** in Kodi. `log.warning()` and `log.debug()` produce zero output in `kodi.log`.

All runtime diagnostics use `xbmc.log()`:

```python
# Correct — appears in kodi.log
xbmc.log("[m3sr2019] message: %s" % value, xbmc.LOGWARNING)

# Silent — never appears in kodi.log
import logging
log = logging.getLogger(__name__)
log.warning("this goes nowhere in Kodi")
```

Kodi log location: `%APPDATA%\Kodi\kodi.log` on Windows. Filter on `[m3sr2019]` to isolate addon output.

---

### Session & Caching

- `_ensure_session()` warms the session before every listing or search call. Called unconditionally — a cookie can exist but be stale/rejected by the server, which a jar-check cannot detect.
- `_cached_get()` validates HTML content before writing to cache (bad pages are never stored) and again on read (poisoned entries are deleted and re-fetched live).
- `_page_has_content()` detects login walls and blank responses using known content signals: `album_report`, `artist_preview`, `tr.song`, `/artist_` hrefs, `__album_` hrefs.
- Cookies are saved after every successful fetch — not just at shutdown, which matters with `reuselanguageinvoker=true`.

---

### Song Search — track_id Reconstruction

The site no longer includes an `id` attribute on `<tr class="song">` elements in search results. The addon reconstructs the token:

```python
# rel is extracted from the play anchor
play_a = song.find("a", rel=True)
rel    = play_a.get("rel", [""])[0]

# track_id is reconstructed — valid because boo() uses track_id[5:] == rel
if not track_id and rel:
    track_id = "song_" + rel
```

Album track listings are unaffected — they always had the `id` on the `<tr>`.

> **Pending:** End-to-end playback from song search results has not been formally verified after this fix. Select a search result and confirm audio plays.

---

### Label Strategy — Skin Compatibility

Aeon Nox Silvo List view renders **one label line per item** — `setLabel2()` and music tag subtitles are never visible. All album and song `ListItem`s embed the artist in the primary label:

```python
li = xbmcgui.ListItem("Artist — Title")
li.setLabel2(artist)        # for skins that do show a subtitle
tag.setTitle("Title")       # plain title only — no artist prefix in metadata
tag.setArtist(artist)
tag.setAlbumArtist(artist)  # required for album-content subtitles in Kodi
```

Applied in: `musicmp3_main_albums`, `artists_albums`, `musicmp3_search` (albums + songs), `favourites()`.

---

### Database

Peewee ORM with SQLite. DB file: `tracks.db` in the addon profile directory.

| Model | Key Fields |
|---|---|
| `Track` | `rel` (unique), `track_id`, `title`, `artist`, `album`, `duration`, `image`, `album_url` |
| `Favourite` | `kind` (album\|artist\|song), `url`, `label`, `thumb`, `artist`, `album`, `added_at` |
| `PageCache` | `url`, `params_hash`, `html`, `expires_at` |

Track inserts are chunked at 100 rows to stay clear of SQLite's 999-variable limit on large albums.
